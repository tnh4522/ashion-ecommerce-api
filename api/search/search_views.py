...
def classify_image_google(image_path):
    try:
        ...
        categories_qs = Category.objects.filter(is_active=True).order_by('sort_order')
        list_category = [category.name for category in categories_qs]
        categories_str = ", ".join(list_category)
        print(categories_str)
        prompt = f"Analyze the object in the provided image and determine its main category. The possible categories are exclusively: {categories_str} and Unknown. If the object does not fit into any categories, it must be classified as Unknown. Please respond with only the category name as a single word, without any additional explanation or text. "
